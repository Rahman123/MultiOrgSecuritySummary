...
export default class MultiOrgSecuritySummary extends NavigationMixin(
  LightningElement
) {
  label = {
    Child_Org_Overview,
    ...
    getSummaries({
      ...
      .then((data) => {
        ...
        this.updateModeAndState(
          data.mode,
          data.summaries[0].healthCheck.multioss__Security_Health_Check_Org__c
        );
  ...
  applyFilters(inSummaries) {
    ...
    for (let isummary in summaries) {
      ...
      if (
        summary.healthCheck != undefined &&
        summary.healthCheck.multioss__Security_Health_Check_Risks__r
      ) {
        for (let irisk in summary.healthCheck
          .multioss__Security_Health_Check_Risks__r) {
          let risk =
            summary.healthCheck.multioss__Security_Health_Check_Risks__r[irisk];
        ...
        if (summary.healthCheck.multioss__Security_Health_Check_Risks__r) {
          this.numberOfRows =
            summary.healthCheck.multioss__Security_Health_Check_Risks__r.length;
        }
      }
  ...
  showHealthCheckDetail(event) {
    ...
    for (let key in this.summaries) {
      if (this.summaries[key].healthCheck.Id === selectedHealthCheckId) {
        this.singleHealthCheck = this.processRisks(
          JSON.parse(JSON.stringify(this.summaries[key]))
        );
        this.singleHealthCheckDate = this.summaries[
          key
        ].healthCheck.multioss__Check_Date__c;
      ...
      let categoryA = [
        {
          ...
          description:
            "These are minor risks that can be updated to improve security.",
          first: true,
          indx: "cat0",
          label: "Informational",
          allRisks: [],
        ...
        },
        {
          category: "LOW_RISK",
          description:
            "Risks in this category are areas to consider for improvement.",
          first: false,
          indx: "cat1",
          label: "Low Risk",
          allRisks: [],
        ...
        },
        {
          ...
          description:
            "Critical Risks in this category should be addressed as soon as possible.",
          first: false,
          indx: "cat2",
          label: "Medium Risk",
          allRisks: [],
        ...
        for (let categoryIndex in categoryA) {
          ...
          for (let ratingIndex in ratingA) {
            if (
              ...
              let currentRisksA =
                categoryA[categoryIndex].ratings[ratingIndex].risks;
              if (currentRisksA == null) {
                currentRisksA = [];
              }

              currentRisksA.push(risk);
              categoryA[categoryIndex].ratings[
                ratingIndex
              ].risks = currentRisksA;
    ...
    for (let key in adminData.SecuritySkills) {
      for (let i in adminData.SecuritySkills[key].Values) {
        let resourceURL =
          ...
          adminData.SecuritySkills[key].Values[
            i
          ].multioss__Image_Path__c.substring(23);
  ...
  showSecuritySyncModal() {
    this.template
      .querySelector("c-multi-org-summary-scheduler")
      .openSyncSchedule();
  }
  ...
  setActiveTab(event) {
    ...
    if (this.tabState !== selectedTab) {
      ...
      if (selectedTab === "details") {
        let selectedRecord = {
          detail: this.selectedOrgData.LatestHealthCheckId
        };
  ...
  get singleAndChildOrgSummaryState() {
    return (
      this.componentState === "childOrgSummary" ||
      this.componentState === "singleOrgSummary"
    );
  }
  ...
  get multiAndChildSummaryState() {
    return (
      this.componentState === "childOrgSummary" ||
      this.componentState === "multiOrgSummary"
    );
  }
